...
public class Client {
	 ...
	 */
	// public static boolean doSend1(String hostname, int port, String query,
	...
	// BufferedReader bufferedreader = new BufferedReader(inputstreamreader);
	// */
	// Log.log(Common.getMethodName(), "FINE", op.toLowerCase() + "ing to " +
	// hostname + ":" + port);
	// Log.log(Common.getMethodName(), "FINE", "SENT: " + query);
	...
	// while (!endFlag) {
	// if (Common.getCurrentSecTimestamp() - start >= Config.CONNECTION_TIMEOUT
	// / 1000) {
	// throw new Exception();
	// }
	// if (in.available() > 0) {
	...
	// // cFile.createNewFile();
	// Log.log(Common.getMethodName(), "FINE", "Downloading to Destination: " +
	// absolutePath);
	// RandomAccessFile downloadingFile = new RandomAccessFile(pathString +
	// filename, "rw");
	//
	...
	// } catch (Exception ee) {
	// // ee.printStackTrace();
	// Log.log(Common.getMethodName(), "FINE", "CONNECTION ERROR: Please check
	// the network or server(" + hostname
	// + ":" + port + "). Timeout or connection refused.");
	// return false;
	// }
	// return true;
