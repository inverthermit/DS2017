...
public class ClientCommandLine {
	...
	public static void main(String[] args){
		String[] testArg={"-query"};
		System.out.println(ClientCommandLine(testArg));
	}
	...
	public static String ClientCommandLine(String[] args) {
		...
		/*
		 * Option channel = Option.builder("channel") .hasArg() .desc("channel")
		 * .build(); opt.addOption(channel);
		 */
		...
		Option description = Option.builder("description").hasArg()
				.desc("rescource description").build();
		opt.addOption(description);
		Option exchange = Option.builder("exchange")
				.desc("exchange server list with server").build();
		opt.addOption(exchange);
		Option fetch = Option.builder("fetch")
				.desc("fetch resources from server").build();
		opt.addOption(fetch);
		Option host = Option.builder("host").hasArg()
				.desc("server host, a domain name or IP address").build();
		opt.addOption(host);
		...
		Option publish = Option.builder("publish")
				.desc("publish resource on server").build();
		opt.addOption(publish);
		Option query = Option.builder("query")
				.desc("query for resource from server").build();
		opt.addOption(query);
		Option remove = Option.builder("remove")
				.desc("remove resource from server").build();
		opt.addOption(remove);
		Option secret = Option.builder("secret").hasArg().desc("secret")
				.build();
		opt.addOption(secret);
		Option share = Option.builder("share").hasArg()
				.desc("share resource on server").build();
		opt.addOption(share);
		Option servers = Option.builder("servers").hasArgs()
				.desc("server list, name1:port1,name2:port2,...").build();
		opt.addOption(servers);
		Option tags = Option.builder("tags").hasArgs()
				.desc("resource tags, tag1,tag2,tag3,...").build();
		opt.addOption(tags);
		Option uri = Option.builder("uri").hasArg().desc("resource URI")
				.build();
		opt.addOption(uri);
		...
		try {
			...
			if (commandline.length == 0) {
				...
				NormalResponse response = new NormalResponse("error",
						error.GENERIC_MISS_INCORRECT);
	...
	public static Resource cliResource(Option[] commandline, int[] command,
			String commandName) {
		Resource resource = new Resource();
	...
	public static void checkArg(int[] arg, int[] command, String commandName) {
		...
		for (int i = 0; i < arg.length; i++) {
			if (command[i] == 1 && arg[i] != command[i]) {
				...
				case "publish":
				case "remove":
					response = new NormalResponse("error",
							error.PUBLISH_REMOVE_RESOURCE_MISSING);
					System.out.println(response.toJSON());
					break;
				case "query":
				case "fetch":
				case "exchange":
					response = new NormalResponse("error",
							error.QUERY_FETCH_EXCHANGE_RESOURCETEMPLATE_MISSING);
					System.out.println(response.toJSON());
					break;
				case "share":
	...
	public static boolean checkString(String str) {
		if (str.contains("\0") || str.charAt(0) == ' '
